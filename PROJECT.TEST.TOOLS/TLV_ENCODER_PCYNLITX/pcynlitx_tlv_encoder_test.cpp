...
int main(int argc, char ** argv){
    ...
    if(argc < 3){
       ...
       std::cout << "\n usage:" << argv[0]
                 ...
                 << " <repitation>  <test binary> <data file path>";
    ...
    std::cout << "\n\n THE PCYNLITX DATA CLUSTERING TEST";
...
void Determine_Test_Command(char ** test_command, char * test_binary, char * data_file_path){
     ...
     int file_path_name_size = strlen(data_file_path);
     ...
     int command_lenght = binary_name_size + record_file_name_size +
                          ...
                          file_path_name_size + execute_command_length;
     ...
     *test_command = new char [5*command_lenght];
     ...
     for(int i=0;i<file_path_name_size;i++){
